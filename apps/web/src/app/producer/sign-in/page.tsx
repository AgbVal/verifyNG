...
export default function ProducerSignInPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data.accountType !== 'PRODUCER' || !data.producer) {
        setError(
          'This sign-in page is for registered VerifyNG producers.',
        );
        return;
      }
    ...
    <main className="min-h-screen bg-white text-[#101828]">
      <header className="border-b border-[#e4e7ec]">
        <div className="mx-auto flex h-20 max-w-[1180px] items-center justify-between px-6">
          ...
          <span className="text-[13px] font-semibold uppercase tracking-[0.12em] text-[#667085]">
            Producer Portal
          </span>
        </div>
      </header>
      ...
      <div className="mx-auto grid min-h-[calc(100vh-80px)] max-w-[1180px] items-center px-6 py-16 lg:grid-cols-2 lg:gap-24">
        <section className="max-w-[520px]">
          ...
          <p className="mt-5 max-w-[480px] text-[16px] leading-7 text-[#667085]">
            Sign in to manage registered products, production
            batches and VerifyNG verification codes.
          </p>
        </section>
        ...
        <section className="mt-12 max-w-[440px] lg:mt-0">
          ...
          >
            ...
            <p className="mt-6 text-center text-sm text-[#667085]">
              Looking for product verification?{' '}
              <Link
                href="/"
